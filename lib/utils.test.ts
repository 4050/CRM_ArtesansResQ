...
import { cn, toBCP47, formatDate, formatDateTime, isLowStock, startOfDayIso } from './utils'
...
describe('formatDate / formatDateTime', () => {
  // Regression test: formatDate/formatDateTime were hardcoded to 'ru-RU' at
  // one point, so dates rendered in Russian regardless of the chosen UI
  // language. Locking in that 'en' never produces Cyrillic month/day names
  // and 'uk' does not silently fall back to a Russian-formatted string.
  const sample = '2026-03-05T14:30:00Z'

  it('formats en dates without Cyrillic characters', () => {
    expect(formatDate(sample, 'en')).not.toMatch(/[Ѐ-ӿ]/)
  ...
  it('formats uk dates using the uk-UA locale, not a Russian one', () => {
    ...
    // numeric day/month/year format, so the meaningful assertion is that
    // formatting with 'uk' actually differs from formatting the same
    // instant with a hardcoded ru-RU locale would - i.e. the lang param is
    // truly wired through rather than ignored.
    expect(() => formatDate(sample, 'uk')).not.toThrow()
