...
-- so a failure partway left some rows already applied with no way for the
-- caller to know which, and no protection against a lost response
-- re-applying an already-succeeded restock on retry. Any row failing here
-- (bad data, a duplicate code, a consumable that no longer exists) rolls
-- back the whole import; the caller gets one clear error naming the first
-- problem.
