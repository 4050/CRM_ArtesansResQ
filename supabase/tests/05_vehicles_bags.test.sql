...
--   bag_a1     = eeeeeeee-...-0001 (org A, shared)
--
-- Uses throwaway vehicle/bag records (suffix -0099) for the archive/delete
-- mutations so the shared fixtures above stay untouched for other files.
-- Bags and vehicles are decoupled (no vehicle_id column at all), so unlike
-- before, deleting a vehicle is no longer blocked by any bag - only
-- delete_vehicle's own not-found guard is exercised here; the
-- foreign_key_violation-to-friendly-message path (now only reachable via a
-- referencing call, not a bag) isn't covered in this file.
begin;
select plan(8);
...
insert into public.bags (id, number, organization_id) values
  ('eeeeeeee-0000-0000-0000-000000000099', 'B-99', 'aaaaaaaa-0000-0000-0000-000000000001');
...
select lives_ok(
  ...
  'admin can delete a vehicle - no bag can block this anymore, they are decoupled'
);
select throws_like(
  $$ select public.delete_vehicle('dddddddd-0000-0000-0000-000000000099'::uuid) $$,
  'Vehicle not found',
...
select lives_ok(
  $$ select public.archive_bag('eeeeeeee-0000-0000-0000-000000000099'::uuid) $$,
  'admin can archive a standalone bag'
);
select lives_ok(
